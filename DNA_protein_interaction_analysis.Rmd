...
The details of data acquisition is explained in the method and here a brief overview of the data is given in addition to some experimental conditions in which data has been acquired. 

## Experimental details

The red laser line with wavelength of 647nm was used to excite the ATTO 647N dyes. 

The illumination laser power in the sample plain were measured to be $0.05- 1 kW/cm^2$. 

The exposure time was 7.5 ms and 23.5 ms for EndoV and hOGG1 respectively. The optimized exposure time for detecting maximum possible signal intensity in maximum lifetime of the fluorescent dyes was decided empirically and after several initial observations of protein-DNA interaction. 

The power of trapping laser in the sample plain was measured to be $1.3-18 mW$.

## Structure of data 

The raw data sets are streams of videos with the length ranging from 20 k frames to 200 k frames. Depending on the sample condition each data set can contain varying numbers of protein-DNA interaction recorded as trajectories of protein scanning along the DNA. 
...
The signal from single molecules were localized in *FIJI* $^1$ using *ThunderSTORM* $^2$ and the camera setup configuration in the in the software was set as: 
...
The output of this analysis were the location of signals registered in time series with the detail of localization, as json files. 

All signal detected in each data set are projected into single images. 
...
Each data set contains some frames that are recorded while the bright field transmission light was on, thus the bead was localized and once the transmission light was turned off and the fluorescence was turned on, the position of bead remained unchanged. Any change in the position of the bead was happening in bright field, therefore trackable. 
...
For each data set beads were localized using *TrackMate* $^3$ plugin in FIJI and the output was saved as XML files. 
...
``` 
The output are exported as RSD files from R and are used for localization of DNA in the next steps.  
...
Here is the list of functions that are sourcing the interface, these functions have been tested on different data sets and they have shown high accuracy in the output however they might not be time efficient and some steps can take a long time. This can be further optimized. 
...
The intermediate output of the code in the last step is the address to the detected trajectories (in terms of frame number) that later will be used to extract the frames in which the interaction is happening.

The script that was used to extract the frames of the events and concatenate them one after another in FIJI: 
...
The result of the image processing and tracking code was the trajectories information which is recorded within three files for each protein as well as three files for bead position registration. These six files are the main input for the upcoming steps. 

From this step onward the outputs of all steps are included along the code in a file titled "Processed". All information for running the code and producing the PDF report is provided in this folder. 
...
3. apply the rotation matrix to rotate the trajectories so that the displacements are mostly projected into x direction. 

4. make a threshold for the fluctuations in y.  

In Fig.5,6 and 8 the trace of the detected trajectories for different data sets color by whether they are within y fluctuation threshold or not.  
...
Based on their fluorescence nature the organic dyes that we have used go to off state for very short periods of time (blinking), thus before going further in the analysis we need to correct for this effect.  
...
The corrected data is saved and ready for further analysis. In order to show the effect of blinking and have a sense of diffusion we plotted 2 trajectories of each protein in Fig.9-14. The steps in the trajectories are colored by the their separation prior to the blinking correction, in other words along with changing the color in each trajectory the dye has been blinking once. The horizontal axis of the plot is frame number and for EndoV and the mutant the frame interval is 7.5 ms and for hOgg1 is 23.5 ms. The vertical axis is the position of the protein along the DNA in terms of nm.  

```{r Showing trajectories, message= FALSE, warning= FALSE, error= FALSE, out.width= '70%', fig.align= "center", fig.cap= "Trajectories"}
...
The binding life time is defined as duration of uninterrupted binding and scanning events. The binding lifetime of different proteins in different salt concentration is shown in Fig.15.  
...
In Fig.16 the distribution of displacements of the protein along DNA within frame intervals is shown. The vertical lines in the graph are 1,2 and 3 standard deviation. 


```{r step size distribution, message= FALSE, warning= FALSE, error= FALSE, out.width= '70%', fig.align= "center", fig.cap="Step size distribution"}
...
CorrectedBlinking$Enzyme <-  factor(CorrectedBlinking$Enzyme, 
                            levels=c('hOgg1','EndoV',
                                     'mEndoV'))
tmp$Enzyme <-  factor(tmp$Enzyme, 
                            levels=c('hOgg1','EndoV',
                                     'mEndoV'))
...
Now we want to investigate whether we see two (or more) different distributions of the diffusion rate or not. But if we look at the average diffusion rate (as has been common in other studies) we do not see any non-singularity in the distribution of the diffusion rate. We need to see the diffusion rate in each moment of time in the trajectories.
...
Using the Rccp function the Instantaneous diffusion rate is calculated and shown for different width of the moving window in Fig.17-19 
...
``` 
Independent of the width of the moving window two peaks of the distribution are visible for hOGG1 and wt EndoV whereas for mutant EndoV there is only one peak. 

Now we calculate the percentage of trajectories that pass the upper theoretical limit of diffusion rate (calculated in 9.3). 
...
For further investigation of the different diffusion modes we use different methods to classify the data. In this part we first classify the data using activation energy barrier and then verify this using Markov hidden states analysis. 

## Activation energy barrier classification 

Assuming the scanning as a chemical reaction, the rate constant is $k=1/t=2D/<x^2>$ and from Arrhenius equation we have $k=Ae^{(-E_a/K_B T)}$. In the case of ideal sliding the $E_a$ is 0, therefore $A = K_i$; plugging these into Arrhenius equation and solving it for $E_a$, we have: $E_a = ln(k_i/k)k_BT$. Given the random walk is happening with the length of 1bp for each step, therefore $<x^2>$ is equal to $1bp^2$, therefore the ratio of $k_i/k$ will be equal to $D_i/D$. Thus for $E_a$ equation we have : $E_a = ln(D_i/D)k_BT$ where $D_i$ is the upper theoretical limit of diffusion for helical sliding. By setting D as instantaneous diffusion rates we can calculate $E_a$ for every step. 

We classify the steps based on the value of $E_a$ into three categories each of which are representative of a particular scanning mode: 
...
The result of this classification is shown in Fig.21.
...
To verify the existence of the three above-mentioned modes of scanning, we used an analytical tool based on a variational Bayesian treatment of hidden Markov models (HMM)$^4$ to classify segments of the trajectories into three states. Later we check the overlap of these three states with that of the energy barrier classification. 
In the first step we need to transform the data to a version that is compatible with the MATLAB code of the HMM software (vbSPT). All trajectories are saved in separate text files and later they are read by MATLAB and concatenate as a cell array. 
...
The next step is performed in MATLAB. By navigating to the same folder containing the text files and running this code for each proteins the cell array files are constructed. 
...
The codes that are used in MATLAB for 3-state classification of the trajectories of the 3 proteins are as following. To perform the classification with higher number of states the variable related to Maximum number of states changes. 
...
The vbSPT software failed to converge on a reasonable number of states, and always selected more than three states to be the best fit for all proteins (8 states for wm- and wt-EndoV, 6 states for hOGG1). According to the publication related the vbSPT software$^4$ the states with forbidden transitions, can acquired transition probabilities up to 0.012, therefore we assume any transition below this value as insignificant and replaced by 0. 


The transition probability matrices for all classification models including 3 or more number of states for all proteins are presented here. As shown here the transition probability matrices of the 4-state classification for wt- and wm-EndoV are nearly reducible and for all other remainder classification models with number of states more than 3 the transition probability matrices are reducible Markov chains. This can be a sign of heterogeneity in our data sets, which might explain why the sbSPT HM modelling predicts more than 3 states for our data sets. In contrast, all the 3-state models have irreducible transition matrices. 

For wt-EndoV: 

3-state: irreducible

4-state: nearly reducible 

5-,6-,7-, 8-states: reducible 
...
Next, the output files for 3-states classifications are transferred into R, and the information of the new classification is added to the rest of data. Here we first look at the distribution of those states in the instantaneous diffusion plot (Fig. 22).
...
Here we show the salt dependence of average diffusion rate of the diffusion modes based on both models of classifications (Fig. 25 and 26). The horizontal lines in the plots show the upper theoretical limit of diffusion rate. 
...
As it is visible the fast mode of EndoV and mutant shows salt dependence and with a good approximation this is consistence for both classification models. 
...
```{r Fig.1c , message= FALSE, error= FALSE, eval=FALSE, warning= FALSE, out.width= '70%'}
...
```
```{r fig1c, echo=FALSE, out.width = '60%', fig.align= "center"}
...
```{r Fig.2a , message= FALSE, error= FALSE, warning= FALSE, fig.asp=0.26, fig.align="center"}
...
```{r Fig.2aInsets , message= FALSE, error= FALSE, warning= FALSE, error= FALSE, fig.align= "center", out.width='30%'}
...
```{r Fig.2b , message= FALSE, error= FALSE, warning= FALSE, fig.asp=0.3, fig.align= "center"}
...
```{r Fig.3a , message= FALSE, error= FALSE, warning= FALSE, fig.align="center", fig.asp= 0.8, out.width='60%' }
